...
{
    assert(s>=0);
    assert(s<m_txt.size()+1);    // MODIF : +1 ajouté pour permettre la sélection d'un menu n'existant pas (éviter d'avoir toujours une case activée)
    m_select=s;
}
...
void Menu::draw(int xmin, int ymin, int xmax, int ymax)
{
    ...
    for(i=0; i<m_txt.size(); ++i)
    {
        if (i==m_select) color(184,25,25);         // MODIF : change la couleur de la case sélectionnée dans le menu
        else color(127,123,122);                    // MODIF : change la couleur des cases non sélectionnées dans le menu
